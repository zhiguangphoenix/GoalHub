...
class GithubClient {
    ...
    func fetchProfile(callback: @escaping (String) -> Void) {
        if user != "" {
            Alamofire.request("https://github.com/\(user)").responseString { response in if response.result.isSuccess {
    ...
    func fetchEvents(callback: @escaping ([String: Int]) -> Void) {
        ...
        func addToEventsDict(json: JSON) {
        ...
        if user != "" {
            ...
            Alamofire.request("\(BASE_URL)/users/\(user)/events\(token != "" ? "?access_token=" + token : "")", headers: headers)
                .responseJSON { response in if let val = response.result.value {
                    let json = JSON(val)
                    
                    addToEventsDict(json: json)
                    
                    Alamofire.request("\(self.BASE_URL)/users/\(self.user)/events\(self.token != "" ? "?page=2&access_token=" + self.token : "?page=2")", headers: headers)
                        .responseJSON { response in if let val = response.result.value {
                            let json = JSON(val)
